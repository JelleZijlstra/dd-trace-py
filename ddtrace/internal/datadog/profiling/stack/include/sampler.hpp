...
class Sampler
{
    ...
    // Rolling window for p_stable: ring buffer of process_delta values (µs CPU per adapt window).
    ...
    // Baseline CPU budget (µs per adapt window) corresponding to an absolute floor overhead.
  ...
  public:
    ...
    // Converted to µs of CPU budget per adaptation window.
