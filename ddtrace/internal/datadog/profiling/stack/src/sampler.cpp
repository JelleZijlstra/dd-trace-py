...
Sampler::adapt_sampling_interval()
{
    ...
    // Where:
    //   baseline = configurable absolute floor (prevents starvation on idle apps, Proposal 2)
    //   o        = target overhead fraction
    //   p_stable = stable (p95) estimate of app CPU usage from rolling window (Proposal 1)
    //
    // The new interval is derived so that sampler CPU time s matches the budget:
    //   I' = I * (s / budget)
